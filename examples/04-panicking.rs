...
fn main() {
    // One of Rust's founding principles is safety. 
    // Given the choice between halting program execution on an unrecoverable error and silently attempting to continue, Rust will choose the former every time.
    ...
    // Panics aren't like exceptions, they aren't designed to be caught or handled (though this functionality does exist...). We will discuss recoverable errors later.
    ...
    // There are also a few flavours of panics that can broadcast a particular intent to the (unfortunate) recipient:

...
fn main() {
    ...
    // This is called a panic. Panics usually occur from doing things like indexing an array out of bounds, etc. 
    // You can invoke your own panic through the panic macro:
    panic!("Sorry, not feeling it today"); 

    // Panics aren't like exceptions from C++, they aren't designed to be caught or handled (though this functionality does exist...). 
    // We will discuss recoverable errors later. Panics are designed for unrecoverable conditions that the program can't work around, 
    // and will cause the program to immediately terminate (after cleaning up it's own memory).

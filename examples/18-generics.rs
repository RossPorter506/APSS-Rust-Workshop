...
fn main() {
    ...
    // impl blocks may also be applied to specific instances of Point. This allows some subtypes of Point to access special functions depending on the type:
    impl Point<f32> {
        fn is_nan(&self) -> bool {
    ...
    // Again, we don't want to have to implement this for f64 too, so we could use the Float trait from the num crate which is defined for all floating point types (f32, f64):

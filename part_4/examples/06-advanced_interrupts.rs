...
// In the previous example we shared an atomic type between the main function and interrupt handler. 
// It wasn't so bad. Unfortunately, we often want to share non-atomic types too! 
// For example, we might want to share a peripheral with the ISR to help handle the interrupt.

// In the following example we'll be wrapping non-atomic types to make them safe to share between 
// interrupt handlers and the main function. The type names can be a bit of a mouthful. Generally 
// in order to share a peripheral, you need to wrap in a Mutex<RefCell<Option< T >>>. Let's break that down:

//              vvvvv - Provides synchronisation for non-atomic types, makes them safe to share. We can't get a &mut T though.
static RED_LED: Mutex<RefCell<Option< Pin<P1, Pin0, Output> >>> = Mutex::new(RefCell::new(None));
//                    ^^^^^^^ - By dynamically checking the borrow rules at runtime it allows us to get a &mut T instead of just &T.

static P2IV: Mutex<RefCell<Option< PxIV<P2> >>> = Mutex::new(RefCell::new(None));
//                         ^^^^^^ - Note we can't configure peripherals at compile time, so we have to put 'None' here now 
//                                  and come back later to put a value in the global variable once it's been configured.
...
fn main() -> ! {
    ...
    //                vv Critical section token. Provided to Mutex to prove our access will be atomic (i.e. no interrupts).
    critical_section(|cs| {
        ...
        //                         ^^^^^^^ - Replace the Option's internals with the provided value: Move red_led into it.
...
fn PORT2() {
    ...
    // The MSP430 does support nested interrupts (if configured), so we still need a 
    // critical section in the interrupt too, just in case.
    critical_section(|cs| {
        ...
        //                                    ^^^^^^ - Get a &mut T if the value is Some(), panic if None. This is fine,
        //                                             because we're *sure* we put values in before enabling interrupts
        ...
        //       ^^^^^^^ - This syntax is basically the same as .as_mut() - don't move the T out of the Option, just give a &mut T.
